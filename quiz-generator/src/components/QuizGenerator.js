...
export default function QuizGenerator({ onGenerate }) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429 || response.status === 402) {
          // Handle both rate limit and payment required errors
          if (errorData && errorData.detail && typeof errorData.detail === 'object' && errorData.detail.upgrade_required) {
            setShowSubscriptionModal(true);
          }
          return;
        }
      ...
      // Update generation limits after successful generation
      await fetchGenerationLimits()

      // Immediately create an initial score attempt (progressive saving)
      let scoreRecord = null;
      try {
        const initRes = await fetch(`${baseUrl}/api/scores/${data.id}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
        ...
        console.error('Initial score create failed', e); 
      }
  ...
  return (
    <div className="flex gap-6 w-full max-w-5xl">
      {/* Main Quiz Generator */}
      <div className="flex-1 glass-card p-8 rounded-2xl" data-testid="quiz-generator" role="form" aria-labelledby="generator-heading">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              disabled={loading || !topic.trim() || !canGenerateQuiz()}
              className={`btn-primary flex-1 ${loading || !canGenerateQuiz() ? 'opacity-75 cursor-not-allowed' : ''}`}
            >
              {loading ? (
                ...
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span>
                    {!canGenerateQuiz() ? 'Upgrade to Generate' : 'Generate Quiz'}
                  </span>
                </div>
              )}
            </button>
